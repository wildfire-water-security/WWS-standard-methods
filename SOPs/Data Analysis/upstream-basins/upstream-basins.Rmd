...
In order to better describe the site and watershed characteristics we often average a metric across the upstream area using a polygon of the area upstream from a site and spatial raster layer. However, obtaining a polygon for the area upstream of a site is not a trivial process.

This script and workflow is used to obtain those upstream area basins for a set of sites using the USGS [StreamStats API](https://streamstats.usgs.gov/ss-delineate/docs) to obtain the basins.
...
Here you'll provide the information needed to run the workflow. This includes:
...
-   `site_csv`: the file path to the `.csv` with the site information or a `data.frame` with site data. The table needs to have the following columns at a minimum:
    ...
    -   `locality`: the stream name or site name
...
-   NHDplus stream flowlines (`.gpkg`): used to check our sites and basins
-   Streamgrid raster (`.tif`): used by StreamStats to perform basin delineations
-   A geopackage (`.gpkg`) of the sites for future use

Then we'll snap our sites to the StreamStats streamgrid which is used for basin delineations. All these steps can be accomplished with the `pre_streamstats` function.
...
If there are points that need to be adjusted, edit the `*-sites_adj.gpkg` shapefile.

Once you've adjusted your points, you'll need to resnap them to the grid, do this using the `snap_streamstats` function.

```{r}
  snap_sites <- snap_streamstats(data_wd = data_wd, 
                                 study_code = study_code)
```
...
We can use R to automatically get the upstream basins using the USGS API. This is done using the following function which takes the snapped points and the two letter code for the state or region. Depending on the number of sites you have, this can take a few minutes.
...
StreamStats' outputs are only as good as the points you give it. While we already checked to make sure they looked like they were in the right spot, let's double check that the correct stream is being delineated.
...
> If any of the basins are incorrect, adjust the point in `*-sites_adj.gpkg` and rerun `snap_streamstats`, `get_streamstats` and `validate_streamstats`.

# Step 3: Get Landscape Characteristics

Once the basins associated with each site have been downloaded and checked you can use these to calculate basin-wide metrics. To automatically get a selection of commonly used metrics (area, elevation, slope, aspect, landuse, precipitation, temperature), you can use the `get_basic_metrics` function. This can take some time so for this example we'll just get elevation. To get information about the metrics, see `file.path(data_wd, "landscape-rasters", "landscape-char-metadata.csv")` for information about the scale and source of the datasets used.

```{r}
get_basic_metrics(data_wd, basins, huc_code, layers="elev")
```

If you have a different raster you want to use, you can do that manually using the `calc_basin_metrics` function to get the average elevation across each upstream area. The function takes four arguments:
...
Since we already have the dataset downloaded, we'll use elevation again.

```{r}
elev <- terra::rast(file.path(data_wd, "landscape-rasters", paste0("elev-", huc_code, ".tif")))
